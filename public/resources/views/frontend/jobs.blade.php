                  ...
                  $('.backupp-sponsored-results').append(`
                     <div id="backupp_jobs_${job.id}" class="job-card result sponsored-job premium-job spon-top" data-job-slug="${job.slug}" data-active="">
                        ...
                        <div class="bottom-container">
                           ...
                           <button name="button" type="submit" class="tertiary SaveJob" SaveJob="${job.slug}" data-label-save="Save">Save</button>
                        </div>
                     </div>
                  `);
               });
...
<script>
   ...
   jQuery(document).on('click', '.QuickApply', function() {
      ...
      var  btn = $(this);
      const jobSlug = btn.attr('QuickApply');
      ...
      $.ajax({
         ...
         success: function(response) {
            ...
            if (response.success) {
               ...
               window.location.href = "{{url('/job/applied-success/12')}}";
            }else{
...
<script>
   ...
   jQuery(document).on('click', '.SaveJob', function() {
      ...
      $.ajax({
         ...
         error: function(xhr, status, error) {
            // Handle errors
            showToast("There was an error saving for the job.", "error");
            console.error('Error:', error);
