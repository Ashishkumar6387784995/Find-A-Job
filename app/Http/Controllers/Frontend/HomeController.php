...
class HomeController extends Controller
{
    ...
    try {
      ...
      if ($jobs) {
        ...
        if ($checkAlreadySavedJobs) {
          ...
          return response()->json([
            'success' => true,
            'message' => 'You have already saves this job',
            'data' => $jobs
          ], 200);
        }
        else {
  ...
  public function jobQuickApplyApi(Request $request)
  {
    try {
      $request->validate([
        'job_slug' => 'required|string'
      ...
      if ($jobs) {
        ...
        $checkAlreadyJobApplication = JobApplication::where('candidate_id', $user->id)->where('job_id', $jobs->id)->first();


        if ($checkAlreadyJobApplication) {
  ...
  public function companyDetailsApi($id)
  {
    try {
      $company = Company::select('companies.*')
        ->with(['getAllCompanySponsers', 'user', 'companySize'])
        ->join('users', 'users.id', '=', 'companies.user_id')
        ->where('users.is_active', '1')
        ->where('users.is_verified', '1')
  ...
  public function jobsAppliedSuccess(Request $request, $slug)
  {
    try {
      ...
      $slug = $slug;
      // print_r($breadcrumbs);die;
      return view('frontend.jobsAppliedSuccess', compact('page_title', 'page_description', 'breadcrumbs', 'slug'));
    } catch (\Exception $e) {
      // dd($e);
      DB::rollback();
      return redirect()->back()->withErrors($e->getMessage());
