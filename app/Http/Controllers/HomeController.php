        ...
        switch ($type) {
            ...
            case 'providertype_status':
                ...
                //web
                  ...
                  case 'web_category_status':
                $category = \App\Models\web\webCategory::find($request->id);
                $category->status = $request->status;
                $category->save();
                break;
            case 'web_category_featured':
                $message_form = __('messages.category');
                $category = \App\Models\web\webCategory::find($request->id);
                $category->is_featured = $request->status;
                $category->save();
            ...
            //shop
            case 'shop_category_status':
                $category = \App\Models\shop\shopCategory::find($request->id);
                $category->status = $request->status;
                $category->save();
                break;
            case 'shop_category_featured':
                $message_form = __('messages.category');
                $category = \App\Models\shop\shopCategory::find($request->id);
                $category->is_featured = $request->status;
                $category->save();
    ...
    public function getAjaxList(Request $request)
    {
        ...
        switch ($request->type) {
            ...
            case 'subcategory':
                ...
                if ($request->has('language_id')) {
                    $languageId = $request->input('language_id');
                    $items = $items->get()->map(function ($subcategory) use ($languageId) {
            ...
            case 'service-list':
                ...
                if ($request->has('language_id')) {
                    $languageId = $request->input('language_id');
                    $items = $items->get()->map(function ($service) use ($languageId) {
                        $translatedName = $service->translate('name', $languageId);
                        $service->text = $translatedName ?: $service->text;
                        return $service;
                    });
                } else {
                    $items = $items->get();
