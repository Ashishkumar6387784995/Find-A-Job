...
?>
{{ html()->form('DELETE', route('store.category.destroy', $data->id))->attribute('data-submit', ' storeCategories'.$data->id)->open() }}
<div class="d-flex justify-content-end align-items-center">
    @if(!$data->trashed())
        @if($auth_user->can('category delete'))
        <a class="me-3 delete-category" href="{{ route('store.category.destroy', $data->id) }}" data--submit=" storeCategories{{$data->id}}" 
            data--ajax="true"
    ...
    @if(auth()->user()->hasAnyRole(['admin']) && $data->trashed())
        <a href="{{ route('store.category.action',['id' => $data->id, 'type' => 'restore']) }}"
        ...
        <a href="{{ route('store.category.action',['id' => $data->id, 'type' => 'forcedelete']) }}"
