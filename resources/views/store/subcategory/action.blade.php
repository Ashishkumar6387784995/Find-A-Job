...
?>
{{ html()->form('DELETE', route('store.subcategory.destroy', $data->id))->attribute('data-submit', 'shopCategory'.$data->id)->open() }}
<div class="d-flex justify-content-end align-items-center">
    @if(!$data->trashed())
        @if($auth_user->can('category delete'))
        <a class="me-3 delete-category" href="{{ route('store.subcategory.destroy', $data->id) }}" data--submit=" shopCategory{{$data->id}}" 
            data--ajax="true"
    ...
    @if(auth()->user()->hasAnyRole(['admin']) && $data->trashed())
        <a href="{{ route('store.subcategory.action',['id' => $data->id, 'type' => 'restore']) }}"
        ...
        <a href="{{ route('store.subcategory.action',['id' => $data->id, 'type' => 'forcedelete']) }}"
