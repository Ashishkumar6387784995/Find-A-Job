...
?>
{{ html()->form('DELETE', route('web.allweb.destroy', $data->id))->attribute('data-submit', 'allshop'.$data->id)->open() }}
<div class="d-flex justify-content-end align-items-center">
    @if(!$data->trashed())
        @if($auth_user->can('category delete'))
        <a class="me-3 delete-category" href="{{ route('web.allweb.destroy', $data->id) }}" data--submit=" allshop{{$data->id}}" 
            data--ajax="true"
    ...
    @if(auth()->user()->hasAnyRole(['admin']) && $data->trashed())
        <a href="{{ route('web.allweb.action',['id' => $data->id, 'type' => 'restore']) }}"
        ...
        <a href="{{ route('web.allweb.action',['id' => $data->id, 'type' => 'forcedelete']) }}"
